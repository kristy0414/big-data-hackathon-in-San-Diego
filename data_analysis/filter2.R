...
parks <- parks[parks$race_eth_code == 9 & parks$geotype == "CT" & !is.na(parks$p_parkacc) & parks$county_name=="San Diego",]
...
fit <- cor(final$Age_Adjusted_Rate, final$Unemployment_rate)
fit
write.csv(final, "data_analysis/mood_unemployment2.csv")
